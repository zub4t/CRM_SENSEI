...
<script src="/CRM_SENSEI_EXTERNAL/menu/menu.js?v3"></script>
...
<link rel="stylesheet" href="/CRM_SENSEI_EXTERNAL/menu/menu.css?v4">
    ...
    <c:forEach items="${structure}" var="s"   varStatus="loop">
        <div class="lvl-0">
            ...
            <c:forEach items="${s.getChildren()}" var="child">
                <a href="http://94.60.89.80:8084/CRM_SENSEI_EXTERNAL/${child.getUrl()}"><div class="lvl-1 not-visible childOf-${child.getParent_id()}" id="${child.getId()}" >${child.getNme()}</div></a>
                </c:forEach>
        </div>
    </c:forEach>
