    ...
    <c:forEach items="${structure}" var="s"   varStatus="loop">
        <div class="lvl-0">
            ...
            <c:forEach items="${s.getChildren()}" var="child">
                <a href="http://mouraazevedo.dyndns.org:8084/CRM_SENSEI/${child.getUrl()}"><div class="lvl-1 not-visible childOf-${child.getParent_id()}" id="${child.getId()}" >${child.getNme()}</div></a>
                </c:forEach>
        </div>
    </c:forEach>
